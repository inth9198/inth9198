...
1. 버튼 설정 제네레이터 구현.<br/>
2. react-dnd를 사용하여 드래그 앤 드롭 작업을 진행하고 관련 훅(hook) 개발.<br/>
3. 섹션별 드롭다운 구역을 설계하고 버튼 그룹 간 이동 및 새로운 그룹 생성 시 구역 설정 로직 구현.<br/>
4. 드래그 애니메이션 및 파란색 화살표를 활용한 위치 예측 기능 개발.<br/>
5. 드래그 중 인덱스 이동 문제 및 스타일 변동 관련 이슈 해결.<br/>
### 버튼 속성 관리<br/>
1. isSingle, lead_only, locked_name 등의 옵션 추가 및 관련 동작 로직 구현.<br/>
2. 특정 조건에서 호버, 이동, 그룹 간 삽입 제한 로직 추가.<br/>
3. 미사용 버튼 영역 생성 및 그룹으로 이동/삭제 기능 구현.<br/>
4.미사용 버튼과 사용 버튼 간 이동 시 발생하는 오류 수정 및 관련 애니메이션 작업.<br/>
### 새 그룹 생성 및 관리<br/>

1. 새 그룹 추가 및 이름 수정 기능 구현.<br/>
2. 그룹 내 아이템의 순서 저장 및 미사용 로직 적용.<br/>
3. 저장 버튼 액션, 히스토리 호출 액션 구현.<br/>
4. 그룹 이동 기능 및 드래그 중 애니메이션 추가.<br/>
5. 비사용 버튼 관련 로직 개선 및 그룹화 기능 정리.<br/>
### 툴팁 및 UX 개선<br/>

1. 사용 및 미사용 버튼의 도움말 툴팁 추가.<br/>
2. 그룹 추가, 이름 수정, 기본값 복원 등에 대한 안내 표시 작업.<br/>
3. 클릭 이벤트와 애니메이션으로 사용자 경험 향상.<br/>

## Gantt Chart 작업<br/>

### Gantt 관련 타입 구성 및 셋업 데이터 설계.<br/>
1. Gantt 제네레이터와 리졸버 구현.<br/>
2. ECGantt 컴포넌트 개발.<br/>
3. Gantt 차트에서 여러 테스크를 한 라인에 표시 가능하도록 수정.<br/>
4. 툴팁 커스터마이징 및 타입 데이터 가공 처리.<br/>
### 최적화 및 코드 정리
<br/>
1. 코드를 스플리팅하고 스트링 형식을 이넘으로 변경하여 유지 보수성을 향상.<br/>
2. dnd 성능 최적화를 위해 throttle 적용.<br/>
3. 디자인 요청에 따른 스타일 정리 및 컴포넌트 속성 정리.<br/>
4. 필요한 로직을 리졸버와 제네레이터로 이관하여 구조를 단순화.<br/>
5. 어트리뷰트 정렬 및 불필요한 항목 필터링 로직 추가.<br/>
6. 이 모든 작업을 통해 이카운트 ERP 시스템의 유지 보수성과 사용자 경험을 향상시키는 데 기여하였습니다.<br/>
